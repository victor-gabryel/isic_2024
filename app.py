...
st.set_page_config(page_title="ISIC 2024 EfficientNet-B0", layout="wide")
...
def predict(img):
    ...
    classe = "MALIGNO" if prob >= THRESHOLD else "BENIGNO"
...
def load_dataset():
    ...
    df["classe"] = df["malignant"].apply(lambda x: "MALIGNO" if x == 1 else "BENIGNO")
...
def find_image(isic):
    ...
    for ext in extensoes:
        result = glob.glob(f"data/**/{isic}.{ext}", recursive=True)
...
st.title("ISIC 2024 - Classificação de Lesões Cutâneas")
...
# Gráficos do projeto, incluindo a distribuição das classes no dataset original e reduzido, curva ROC-AUC, evolução da ROC-AUC durante o treinamento, evolução da Loss e matriz de confusão.
st.divider()
...
graficos = [
    ...
    ("Evolução da ROC-AUC", "roc_auc.png"),
...
for index, (nome, arquivo) in enumerate(graficos):
    ...
    with coluna:
        if os.path.exists(caminho):
            ...
            st.warning(f"Gráfico não encontrado: {arquivo}")
...
opcao = st.selectbox(
    "Selecionar",
    ["10 imagens aleatórias", "10 benignas", "10 malignas", "5 benignas + 5 malignas"],
)
...
def escolher():
    ...
    return pd.concat([df[df.malignant == 0].sample(5), df[df.malignant == 1].sample(5)])
...
if st.session_state.get("executar"):
    ...
    linhas = [imagens.iloc[i : i + 5] for i in range(0, len(imagens), 5)]

    for linha in linhas:
        cols = st.columns(5)
        for col, (_, row) in zip(cols, linha.iterrows()):

            path = find_image(row.isic_id)
            ...
            if path:
                ...
                resultado = "ACERTO ✅" if pred == row.classe else "ERRO ❌"
